...
EnrichNet <- function(genelist, network, p = 0, TERM2GENE = NULL,
                      TERM2NAME = NULL, threshold = 1e-9, n = 10, nperm = 1){
	...
	resmat <- data.frame(set_names, xd_vec, rep(length(genes),length(set_indices)), path_lengths, overlaps) 	
...
#     ########### pia
#     path_genes <- intersect(TERM2GENE[, 2], names(u))
#     u_pathway <- 1 - u[path_genes]
#     distance_allpath <- data.frame(
#         u_pathway = sort(u_pathway),
#         ranks = ntile(seq_len(length(u_pathway)), n))
#     distance_path_list <- split(rownames(distance_allpath), distance_allpath[, 2])
#     pia <- lapply(distance_path_list, function(x) {
...
#     #### filter
#     pathLen <- unlist(lapply(distance_scores, function(x) {length(intersect(names(x), names(genelist)))}))
#     distance_scores <- distance_scores[which(pathLen > 0)]
#     PATHID2EXTID <- PATHID2EXTID[which(pathLen > 0)]
#     ###########
#     Xd <- lapply(distance_scores, get_Xd, pia = pia, n = n,
#         genelist = genelist, u = u, TERM2GENE = TERM2GENE)
#     names(Xd) <- names(PATHID2EXTID)
#     if (nperm == 1) {
#         result <- data.frame(id = names(Xd), Xd =  unlist(Xd))
